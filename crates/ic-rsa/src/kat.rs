...
///
/// This still says `agentic-crypto`, which was the project's name before it
/// was renamed to IronCrypto, and it is left that way deliberately. The
/// signatures below are pinned to this exact input. Changing the message would
/// force all six to be regenerated from the implementation they are supposed to
/// be watching, and a rename is the worst possible excuse for re-baselining a
/// regression anchor: it would destroy the evidence that the rename changed no
/// cryptographic output, which is the one thing worth knowing afterwards.
///
/// The string is opaque test input, not a name anybody reads.
        ...
        impl SelfTest for $scheme {
            fn self_test() -> Result<()> {
                ...
                <$scheme>::sign(&kat_key()?, KAT_MESSAGE, &mut got)?;
                    ...
                    ic_core::ct::verify(&unhex_256($expected)?, &got),
                    SelfTestFailed,
                    $id
                );
                <$scheme>::verify(&kat_public()?, KAT_MESSAGE, &got)
                    .map_err(|_| ic_core::err!(SelfTestFailed, $id))
            }
        }
    };
}
...
pss_kat!(PssSha256, ic_hash::Sha256, KAT_PSS_SHA256, "rsa-pss-sha256");
pss_kat!(PssSha384, ic_hash::Sha384, KAT_PSS_SHA384, "rsa-pss-sha384");
pss_kat!(PssSha512, ic_hash::Sha512, KAT_PSS_SHA512, "rsa-pss-sha512");
...
mod tests {
    ...
    /// `cargo test -p ic-rsa -- --ignored --nocapture print_kat`.
    #[test]
    ...
    fn print_kat_signatures() {
        ...
        pss!(ic_hash::Sha256, "KAT_PSS_SHA256");
        pss!(ic_hash::Sha384, "KAT_PSS_SHA384");
        pss!(ic_hash::Sha512, "KAT_PSS_SHA512");
    }
}
