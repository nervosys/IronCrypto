...
use ac_core::traits::{Algorithm, Digest, SelfTest, SignatureScheme};
...
impl SignatureScheme for Ed25519 {
    ...
    fn verify(public_key: &[u8], message: &[u8], signature: &[u8]) -> Result<()> {
        ...
        let a_point = Point::decompress(&pk_bytes).ok_or(ac_core::err!(
            MalformedEncoding,
            "ed25519 public key is not on the curve"
        ))?;
        let r_point = Point::decompress(&big_r).ok_or(ac_core::err!(
            MalformedEncoding,
        ...
        if ac_core::ct::verify(&lhs.compress(), &rhs.compress()) {
            Ok(())
        } else {
            Err(ac_core::err!(AuthenticationFailed, "ed25519"))
        }
    }
}
...
impl SelfTest for Ed25519 {
    fn self_test() -> Result<()> {
        ...
        <Self as SignatureScheme>::sign(&seed, b"", &mut sig)?;
            ...
            ac_core::ct::verify(&want_sig, &sig),
            SelfTestFailed,
            "ed25519"
        );
