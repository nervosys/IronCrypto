...
///
///     cargo test --release -p ic-cipher --test throughput -- --ignored --nocapture
#[test]
#[ignore]
fn measure() {
    ...
    for (label, cipher) in [
        (
            ...
            ic_cipher::Aes256::new(&[0x2a; 32]).unwrap(),
        ),
        (
            "aes-256 blocks (portable)",
            ic_cipher::Aes256::new_portable(&[0x2a; 32]).unwrap(),
        ),
    ] {
        let t = Instant::now();
    ...
    let gcm = ic_cipher::Aes256Gcm::new(&[0x2a; 32]).unwrap();
    let mut tag = [0u8; 16];
    let t = Instant::now();
    ...
    let cc = ic_cipher::ChaCha20Poly1305::new(&[0x2a; 32]).unwrap();
    let t = Instant::now();
    ...
    println!("aes backend: {:?}", ic_cipher::aes::active_backend());
    println!("ghash accelerated: {}", ic_cipher::gcm::ghash_accelerated());
}
