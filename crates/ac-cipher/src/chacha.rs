...
//! `fips_status: NotApproved`, and `ac-fips` refuses to construct them while
//! the module is in approved mode.

use ac_core::traits::{Aead, Algorithm, Mac, SelfTest};
...
pub fn chacha20_xor(key: &[u8], nonce: &[u8], counter: u32, data: &mut [u8]) -> Result<()> {
    ...
    for (i, chunk) in data.chunks_mut(64).enumerate() {
        let ctr = counter
            ...
            .ok_or(ac_core::err!(CounterExhausted, "chacha20 block counter"))?;
...
impl SelfTest for Poly1305 {
    fn self_test() -> Result<()> {
        ...
        let mut want = [0u8; 16];
        ac_core::codec::hex_decode(b"a8061dc1305136c6c22b8baf0c0127a9", &mut want)?;
        ensure!(ac_core::ct::verify(&want, &tag), SelfTestFailed, "poly1305");
        Ok(())
    }
}
...
impl Aead for ChaCha20Poly1305 {
    ...
    fn open_detached(&self, nonce: &[u8], aad: &[u8], in_out: &mut [u8], tag: &[u8]) -> Result<()> {
        ...
        if !ac_core::ct::verify(&expected, tag) {
            in_out.zeroize();
            return Err(ac_core::err!(AuthenticationFailed, "chacha20-poly1305"));
        }
        chacha20_xor(&self.key, nonce, 1, in_out)
    }
}
