    ...
    ($kw:ident, $kwp:ident, $cipher:ty, $key_len:literal, $kw_id:literal, $kwp_id:literal) => {
        ...
        impl $kw {
            ...
            pub fn unwrap(kek: &[u8], ciphertext: &[u8], out: &mut [u8]) -> Result<()> {
                ...
                let ok = ac_core::ct::verify(&a, &KW_IV);
                if !ok {
                    for block in r.iter_mut() {
                        block.zeroize();
                    }
                    return Err(ac_core::err!(AuthenticationFailed, $kw_id));
                }
                for i in 0..n {
        ...
        impl $kwp {
            ...
            pub fn unwrap(kek: &[u8], ciphertext: &[u8], out: &mut [u8]) -> Result<usize> {
                ...
                if plausible {
                    ...
                    ok = ok.and(ac_core::ct::is_zero(&[zeros]));
                }
                ...
                if !bool::from(ok) {
                    ...
                    return Err(ac_core::err!(AuthenticationFailed, $kwp_id));
                }
...
impl SelfTest for Aes128Kw {
    ...
    fn self_test() -> Result<()> {
        let mut kek = [0u8; 16];
        ac_core::codec::hex_decode(b"000102030405060708090a0b0c0d0e0f", &mut kek)?;
        let mut key = [0u8; 16];
        ac_core::codec::hex_decode(b"00112233445566778899aabbccddeeff", &mut key)?;
        let mut want = [0u8; 24];
...
impl SelfTest for Aes192Kwp {
    ...
    fn self_test() -> Result<()> {
        ...
        let mut key = [0u8; 20];
        ac_core::codec::hex_decode(b"c37b7e6492584340bed12207808941155068f738", &mut key)?;
        let mut want = [0u8; 32];
        ac_core::codec::hex_decode(
        ...
        let len = Aes192Kwp::unwrap(&kek, &want, &mut back)?;
            ...
            ac_core::ct::verify(&key, &back[..len]),
            SelfTestFailed,
            "aes-192-kwp"
        );
...
impl SelfTest for Aes256Kwp {
    ...
    fn self_test() -> Result<()> {
        ...
        let len = Aes256Kwp::unwrap(&kek, &wrapped, &mut out)?;
            ...
            ac_core::ct::verify(secret, &out[..len]),
            SelfTestFailed,
            "aes-256-kwp"
        );
