...
//! the ontology as [`Experimental`][ac_ontology_status], not `Available`, and
...
//!
//! [ac_ontology_status]: https://docs.rs/ac-ontology
    ...
    ($name:ident, $cipher:ty, $key_len:literal, $id:literal, $disp:literal) => {
        ...
        impl Aead for $name {
            ...
            fn open_detached(
                ...
                let ok = ac_core::ct::verify(&s, tag);
                s.zeroize();
                if !ok {
                    // Never hand back unauthenticated plaintext.
                    in_out.zeroize();
                    return Err(ac_core::err!(AuthenticationFailed, $id));
                }
                Ok(())
            }
        }
