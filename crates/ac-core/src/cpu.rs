...
//!
//! Lives in `ac-core` so that both the implementation crates and the ontology
//! can consult it without either depending on the other: `ac-cipher` uses it to
//! pick a backend, and `ac-ontology` uses it to tell an agent which backend is
//! live.
//!
