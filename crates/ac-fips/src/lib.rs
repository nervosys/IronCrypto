//! # ac-fips — the FIPS 140-3 module boundary
//!
...
//! call was approved.
//!
//! **It is not a validated module.** AgenticCrypto holds no CMVP certificate,
...
//! [`ac_ontology::runtime::has`] with `"fips-validated"`, which returns `false`
...
//!
//! ```
//! use ac_fips::{initialize, Mode, ServiceIndicator};
//!
...
use ac_core::{ensure, Error, ErrorKind, Result};
use ac_ontology::{FipsStatus, ImplStatus};
...
pub fn check(algorithm_id: &str) -> Result<ServiceIndicator> {
    match state() {
        ...
        _ => {
            return Err(Error::new(
                ErrorKind::ModuleErrorState,
                "module not initialized; call ac_fips::initialize()",
            ))
        }
    }

    let entry = ac_ontology::get(algorithm_id)
...
///
/// ```
/// use ac_fips::{guarded, initialize};
/// use ac_core::traits::Digest;
///
/// initialize()?;
/// let (digest, indicator) = guarded("sha2-256", || ac_hash::Sha256::digest(b"data"))?;
/// assert_eq!(indicator, ac_fips::ServiceIndicator::Approved);
/// # let _ = digest;
/// # Ok::<(), ac_core::Error>(())
/// ```
...
mod tests {
    ...
    fn validation_statement_denies_certification() {
        ...
        assert!(!ac_ontology::runtime::has("fips-validated"));
    }
}
