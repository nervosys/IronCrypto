...
//!
//! ```text
//! cargo test -p ic-pkix --release --test fuzz -- --ignored --nocapture
//! ```
//!
//! # The properties
//!
...
use ic_pkix::{der, ecdsa_signature, pem, PrivateKeyInfo, PublicKeyInfo};
...
/// for. Using [`ic_drbg`] here would be slower and would make this test depend
    ...
    for _ in 0..4_000 {
        ...
        if prng.below(2) == 0 {
            ...
            for chunk in input.chunks(48) {
                let mut line = vec![0u8; ic_core::codec::base64_encoded_len(chunk.len())];
                ic_core::codec::base64_encode(chunk, &mut line).unwrap();
