...
use ac_core::traits::{Algorithm, Drbg, Mac, SelfTest};
...
pub type HmacDrbgSha256 = HmacDrbg<ac_mac::HmacSha256>;
/// HMAC_DRBG over HMAC-SHA-512 (256-bit security strength).
pub type HmacDrbgSha512 = HmacDrbg<ac_mac::HmacSha512>;
...
impl SelfTest for HmacDrbgSha256 {
    fn self_test() -> Result<()> {
        ...
        let mut want = [0u8; 64];
        ac_core::codec::hex_decode(SELF_TEST_KAT.as_bytes(), &mut want)?;
        ensure!(
            ac_core::ct::verify(&want, &out),
            SelfTestFailed,
            "hmac-drbg-sha2-256"
        );
        Ok(())
