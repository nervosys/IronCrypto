...
//! [`HmacDrbg`] seeded from [`ac_core::entropy`], tracks the reseed interval,
...
use ac_core::traits::{Drbg, RandomSource};
use ac_core::{Result, Zeroize};
...
impl Rng {
    ...
    pub fn from_os() -> Result<Self> {
        ...
        ac_core::entropy::fill(&mut seed)?;
        ac_core::entropy::fill(&mut nonce)?;
        let drbg = HmacDrbgSha256::instantiate(&seed, &nonce, b"AgenticCrypto/Rng")?;
        seed.zeroize();
        nonce.zeroize();
        Ok(Self {
            drbg,
