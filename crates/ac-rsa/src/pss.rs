...
use ac_core::traits::{Digest, RandomSource};
use ac_core::{ct, ensure, Result};
...
pub fn verify<H: Digest>(key: &RsaPublicKey, message: &[u8], signature: &[u8]) -> Result<()> {
    ...
    if key.raw_public(signature, &mut recovered[..size]).is_err() {
        return Err(ac_core::err!(AuthenticationFailed, "rsa signature"));
    }
...
mod tests {
    ...
    fn rng(label: &[u8]) -> ac_drbg::Rng {
        ac_drbg::Rng::from_entropy(&[0x42u8; 32], label).unwrap()
    }
