...
use ac_cipher::aes::{Aes128, Aes192, Aes256, BLOCK_LEN};
use ac_core::traits::{Algorithm, BlockCipher, Mac, SelfTest};
    ...
    (
        ...
        impl SelfTest for Cmac<$inner> {
            fn self_test() -> Result<()> {
                ...
                ac_core::codec::hex_decode($kat_tag.as_bytes(), &mut want)?;
                key.zeroize();
                ensure!(ac_core::ct::verify(&want, &tag), SelfTestFailed, $id);
                Ok(())
            }
        }
    };
