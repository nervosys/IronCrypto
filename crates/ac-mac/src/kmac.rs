...
use ac_core::traits::{Algorithm, SelfTest};
use ac_core::{ensure, Result};
use ac_hash::sp800_185::{right_encode, MAX_ENCODE};
    ...
    ($name:ident, $cshake:ty, $id:literal, $disp:literal, $bits:literal) => {
        ...
        impl $name {
            ...
            pub fn verify(key: &[u8], custom: &[u8], data: &[u8], tag: &[u8]) -> Result<()> {
                ...
                Self::mac(key, custom, data, &mut expected[..tag.len()]);
                    ...
                    ac_core::ct::verify(&expected[..tag.len()], tag),
        ...
        impl SelfTest for $name {
            ...
            fn self_test() -> Result<()> {
                ...
                ensure!(ac_core::ct::verify(&short, &again), SelfTestFailed, $id);
...
kmac!(Kmac128, ac_hash::CShake128, "kmac128", "KMAC128", "128");
kmac!(Kmac256, ac_hash::CShake256, "kmac256", "KMAC256", "256");

#[cfg(test)]
mod tests {
    use super::*;
    use ac_hash::sp800_185::{left_encode, CShake128, CShake256};
    ...
    /// `left_encode` is re-exported through `ac_hash`; make sure the path the
